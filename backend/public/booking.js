...
function renderTimeSlots(slots, date) {
    ...
    // Format date for display - parse in local timezone to avoid timezone shift
    ...
    if (slots.length === 0) {
        ...
        if (dateIndex > -1) {
            ...
            // Re-render calendar to remove the blue dot
            renderCalendar();
        }
    } else {
        slots.forEach(time => {
...
async function handleFormSubmit(e) {
    ...
    if (!formData.client_name || !formData.address || !formData.email || !formData.phone) {
    ...
    try {
        ...
        if (data.success) {
            showSuccess(`Appointment confirmed! You will receive a confirmation email at ${formData.email}`);
